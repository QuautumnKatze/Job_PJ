        ...
        <section class="signup-screen-sec">
            <div class="container">
                <div class="signup-screen" style="margin:0 auto 0">
                    ...
                    <form id="registerForm" action="{{route('homepage.register.submit')}}" method="POST">
                        @csrf
                        <div class="mb-2">
                        ...
                        <div class="mb-2">
                            <select id="defaultSelect" name="gender" class="form-control" style="margin-bottom:0">
                        ...
                        <span>Đã có tài khoản? <a href="{{route('homepage.login')}}"> Đăng nhập</a></span>
                    </form>
                </div>
            </div>
        </section>
        ...
        <script>
            $(document).ready(function () {
                $('#email').on('input', function () {
                    ...
                    if (email.length > 0) {
                        $.ajax({
                            url: '{{ route('homepage.check.email') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                email: email
                ...
                $('#phone').on('input', function () {
                    ...
                    if (phone.length > 0) {
                        $.ajax({
                            url: '{{ route('homepage.check.phone') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                phone: phone
                ...
                $('#user_name').on('input', function () {
                    ...
                    if (user_name.length > 0) {
                        $.ajax({
                            url: '{{ route('homepage.check.username') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                user_name: user_name
